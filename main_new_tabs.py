...
tab1, tab2, tab3 = st.tabs(['NWSS Bull Background Information & Trends', 'NWSS Bull Division Analysis', 'NWSS '
                                                                                                        'Selected '
                                                                                                        'Bull '
                                                                                                        'Analysis'])
...
with tab1:
    ...
    # st.subheader('All Bulls Category Correlations')
    # col_hm1, col_hm2 = st.columns([4, 2], gap='large')

    # df_cor_col = df[['Days_Old', 'REA', 'REA_per_100', 'Weight', 'ADG', '%IMF', 'Backfat', 'Scrotal', 'Meas_Year']]
    #
    # with col_hm1:
    #     st.write(correlation_observations)
    #     st.write(df_cor_col.corr())
    #
    # with col_hm2:
    #
    #     df_corr = df_cor_col.corr()
    #     plot = px.imshow(df_corr,
    #                      color_continuous_scale='Blues')
    #     st.plotly_chart(plot)

# TREND EXPERIMENT

# mean_list = df[['Days_Old', 'REA', 'REA_per_100', 'Weight', 'ADG', '%IMF', 'Backfat', 'Scrotal']]
#
# # df_test = df[df['Class'] == col]
# for col in div_list:
#     samp = df[df['Class'] == col]
#     samp = samp.reset_index(drop=True)
#     samp.index += 1
#     st.subheader(f'{col}')
#     # st.markdown(f'{samp}')
#     for cat in mean_list:
#         cat_mean = samp[cat].mean().round(2)
#         samp['cat'] = cat_mean
#         st.write(f'Mean - {cat}:  {cat_mean}')
#         # st.write(f'{samp}')
#
#         #     mean = df[sample].mean()
#         #     st.write(mean)
#         # plot4 = px.box(data_frame=samp,
#         #                    x=cat,
#         #                    y=samp.index,
#         #                    color='Meas_Year',
#         #                    title='Test')
...
with tab2:
    ...
    with col_pd2:
        ...
        cat_y = st.selectbox(label="Select Y Category",
                             key='caty',
                             options=cat_choices,
    ...
    # st.subheader('{} Category Correlations'.format(dropdown_1))
    # col_hm1, col_hm2 = st.columns([4, 2], gap='small')
    #
    # df_cor_col = df_plot[['Days_Old', 'REA', 'REA_per_100', 'Weight', 'ADG', '%IMF', 'Backfat', 'Scrotal', 'Meas_Year']]
    #
    # with col_hm1:
    #     st.write(df_cor_col.corr().round(2))
    #
    # with col_hm2:
    #     df_corr = df_cor_col.corr().round(2)
    #     plot = px.imshow(df_corr,
...
with tab3:
    ...
    st.divider()
    # div_col1, div_col2, div_col3, div_col4, div_col5 = st.columns(5, gap='medium')
    #
    # with div_col1:
    #     st.subheader('REA Metrics')
    #
    #     div_max_rea = div['REA'].max()
    #     div_min_rea = div['REA'].min()
    #     div_mean_rea = (div['REA'].mean()).round(2)
    #
    #     st.write(div_max_rea)
    #     st.write(div_min_rea)
    #     st.write(div_mean_rea)
    # with div_col2:
    #     st.subheader('REA Ratio Metrics')
    #
    #     div_max_rea_ratio = div['REA_per_100'].max()
    #     div_min_rea_ratio = div['REA_per_100'].min()
    #     div_mean_rea_ratio = (div['REA_per_100'].mean()).round(2)
    #
    #     st.write(div_max_rea_ratio)
    #     st.write(div_min_rea_ratio)
    #     st.write(div_mean_rea_ratio)
    # with div_col3:
    #     st.subheader('Weight Metrics')
    #
    #     div_max_Weight = div['Weight'].max()
    #     div_min_Weight = div['Weight'].min()
    #     div_mean_Weight = (div['Weight'].mean()).round(2)
    #
    #     st.write(div_max_Weight)
    #     st.write(div_min_Weight)
    #     st.write(div_mean_Weight)
    # with div_col4:
    #     st.subheader('%IMF Metrics')
    #
    #     div_max_IMF = div['%IMF'].max()
    #     div_min_IMF = div['%IMF'].min()
    #     div_mean_IMF = (div['%IMF'].mean()).round(2)
    #
    #     st.write(div_max_IMF)
    #     st.write(div_min_IMF)
    #     st.write(div_mean_IMF)
    # with div_col5:
    #     st.subheader('ADG Metrics')
    #
    #     div_max_ADG = div['ADG'].max()
    #     div_min_ADG = div['ADG'].min()
    #     div_mean_ADG = (div['ADG'].mean()).round(2)
    #
    #     st.markdown(div_max_ADG)
    #     st.markdown(div_min_ADG)
    #     st.markdown(div_mean_ADG)

    # Plotting the data
    # bull_rea_data = bull['REA']
    # df[df.Letters=='C'].Letters.item()
    # test = bull[bull.REA==11.21].REA.item()
    # print(test)
    # bull_rea = bull['REA'].astype('int')

    # rea_data = [bull_rea, div_max_rea, div_max_rea, div_min_rea]
    # labels = [bull['Name'], 'R1','R2','R3']
    ...
    # with col5:
    #     # st.write(bull['REA'])
    #     trace1 = px.scatter(data_frame=bull,
    #                         y='REA',
    #                         x='Name',
    #                         size='REA',
    #                         title='Selected Bull REA Size').update_layout(yaxis_title="REA Size in Inches")
    #
    #     st.plotly_chart(trace1)
    #
    # with col6:
    #     trace1 = px.box(data_frame=div,
    #                     y='REA',
    #                     title='Division Metrics for {} Size'.format('REA'))
    #     st.plotly_chart(trace1)
    #
    # # REA per 100 Charts
    # with col5:
    ...
    #                         x='Name',
    #                         size='REA',
    #                         title='Selected Bull REA per 100').update_layout(yaxis_title="REA per 100 in Inches")
    #
    #     st.plotly_chart(trace1)
    #
    # with col6:
    #     trace1 = px.box(data_frame=div,
    #                     y='REA_per_100',
    #                     title='Division Metrics for {} Size'.format('REA_per_100'))
    #     st.plotly_chart
