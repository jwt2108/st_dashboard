...
##
# """This python script is the baseline for the STREAMLIT AHCA Bull Dashboard
#  - It ingests the appropriate .csv file for analytics
#  - Reindexes the columns so they make sense
#  - The Dashboard is basically a display of selected analytics that apply to the AHCA Bull Data
#  - There 5 tabs
#     -- Background Info
#     -- Division Trends
#     -- Division Analysis
#     -- Selected Bull Analysis
#     -- How old is my bull?
#  """
#
# """TODO1: Bull registration numbers
#     - Where are they?
#     - Set up a lookup to the AHCA database - just retrieve the page in a new window
#     TODO2: How old is my bull?
#     - Once the age is figured out in days return the appropriate division data
#         -- Figure which division the age falls in at that time
#         -- Present the division and the spreadsheet below
#         -- Need queried bull and the division box plot for:
#             --- Age in Days
#             --- Weight
#             ---
#
...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(['NWSS Highland Bull Background Information    ',
...
with tab1:
    ...
    st.markdown("<h1 style='text-align: center; color: CornflowerBlue;'>Background Information</h1>",
    ...
    c2 = st.container()
    with c2:
        st.markdown("<h1 style='text-align: center; color: CornflowerBlue;'>Average Daily Gain (ADG)</h1>",
                    unsafe_allow_html=True)
...
with tab3:
    ...
    col7, col1, col2, col3, col4, col5, col6 = st.columns(7, gap='small')

    col1.caption(f'Age(Days Old)')
    text = df_plot['Days_Old'].mean().round(2)
    text_age_min = df_plot['Days_Old'].min().round(2)
    text_age_max = df_plot['Days_Old'].max().round(2)

    col1.subheader(f':violet[Avg: {text} ]')
    col1.subheader(f':violet[Min Age: {text_age_min}]')
    col1.subheader(f':violet[Max Age: {text_age_max}]')

    col2.caption('Weight')
    text2 = df_plot['Weight'].mean().round(2)
    text2_wt_min = df_plot['Weight'].min().round(2)
    text2_wt_max = df_plot['Weight'].max().round(2)

    col2.subheader(f':violet[Avg Weight: {text2}]')
    col2.subheader(f':violet[Min Weight: {text2_wt_min}]')
    col2.subheader(f':violet[Max  Weight: {text2_wt_max}]')
    ...
    cat_choices = ['bull_score', 'Weight', 'REA', 'ADG', 'REA_per_100', '%IMF', 'Days_Old', 'Backfat', 'Scrotal', 'DOB',
                   'Meas_Year']
...
with tab5:
    ...
    with age3:
        ...
        st.markdown(f"<h2 style='text-align: center; color: Cyan;'>Age in Days = {age_in_days}</h2>",
                    unsafe_allow_html=True)


    # Set up Divisions by Days Old
    # """
    # Intermediate Bull Calf 154 - 268
    # Junior Bull Calf 269 - 394
    # Senior Bull Calf 395 - 510
    # Intermediate Yearling Bull 511 -633
    # Junior Yearling Bull 634 -785
    # Senior Bull 786 - 1118
    #

    def get_division(age):
        if 154 <= age <= 268:
            division = 'Intermediate Bull Calf'
        elif 269 <= age <= 394:
            division = 'Junior Bull Calf'
        elif 395 <= age <= 510:
            division = 'Senior Bull Calf'
        elif 511 <= age <= 633:
            division = 'Intermediate Yearling Bull'
        elif 634 <= age <= 785:
            division = 'Junior Yearling Bull'
        elif 786 <= age <= 1300:
            division = 'Senior Bull'
        else:
            division = 'Not a division age'
        return division


    nwss_division = get_division(age_in_days)

    print(nwss_division)
    if age_in_days > 1:
        show_sel_bull = True
        if show_sel_bull:
            st.markdown(f"<h4 style='text-align: center; color: CornflowerBlue;'>Your bull is {age_in_days} Days Old "
                        f"and 'age matches' for "
                        f"the {nwss_division} Division</h4>", unsafe_allow_html=True)
            # st.markdown("Your bull is {} days old and age matches 'age in days' for the {} Division".format(age_in_days,
            #                                                                                                 nwss_division))

            df_division = df[df['Class'] == nwss_division]

            st.subheader(nwss_division)
            my_bull_co1, my_bull_col2 = st.columns([2, 1])
            with my_bull_co1:
                my_bull = px.scatter(data_frame=df_division,
                                     x='Days_Old',
                                     y='Weight',
                                     size='Weight',
                                     color='Meas_Year',
                                     hover_name='Name',
                                     title='{} Scatter Plot - Days Old v. Weight'.format(nwss_division)

                                     )
                st.plotly_chart(my_bull)
            with my_bull_col2:
                my_bull_box = px.box(data_frame=df_division,
                                     y='Weight',
                                     title='{} Box Plot - Weight Statistics'.format(nwss_division)
                                     )
                st.plotly_chart(my_bull_box)
            st.write(df_division)
...
with tab6:
    ...
    bull_name = st.text_input(label='Bull Name',
                              help='Used only for report output')
    fcc1, fcc2, fcc3, fcc4, fcc5 = st.columns(5)
    with fcc1:

        start_wt_entry = st.date_input(label='Start Weight Date',
                                       help='Enter Last Weight Check Date (YYYY/MM/DD)',
                                       format="YYYY/MM/DD",
                                       )
    with fcc2:
        stop_wt_entry = st.date_input(label='Current Weight Date',
                                      help='Enter Current Weight Check Date (YYYY/MM/DD)',
                                      format="YYYY/MM/DD",
                                      )

    # number = st.number_input("Integer", 0, 10, "min", 1)
    with fcc3:
        start_weight = st.number_input('Enter Start Weight', 0, 3000, 'min', 1,
                                       help='Enter Weight from Starting Weight Check',
                                       )
    with fcc4:
        stop_weight = st.number_input('Enter Current Weight', 0, 3000, 'min', 1,
                                      help='Enter Weight from Current Weight Check',
                                      )
    with fcc5:
        daily_ration = st.number_input('Daily Ration in Pounds', 0, 80, 'min', 1,
                                       help='Enter the daily ration in pounds (Feed weight per day)', )

    number_of_days = stop_wt_entry - start_wt_entry
    number_of_days = number_of_days.days

    if stop_weight >> 0 and start_weight >> 0:
        fcc_gain = (stop_weight - start_weight)
        # st.write('Number of days between weight checks: {} Days '.format(number_of_days))
        # st.write('Weight Gain over Requested Date Range: {} lb.'.format(fcc_gain))

        if daily_ration >> 0:
            total_ration = daily_ration * number_of_days
            # st.write('Total Feed Consumed during Requested Date Range: {}'.format(total_ration))

            if fcc_gain >> 0 and number_of_days >> 1 and daily_ration >> 0:
                adg = (fcc_gain / number_of_days).__round__(2)
                # st.write('Average Daily Gain (ADG) for Requested Date Range: {}'.format(adg))
                fcr = (total_ration / fcc_gain).__round__(2)
                # st.write('Feed Conversion Rate: {}'.format(fcr))
                ...
                with row1_c3:
                    ...
                    csv_df = pd.DataFrame({'Name: ': bull_name,
                                           ...
                                           'End Date:': stop_wt_entry,
                                           'Report Period:': number_of_days,
                                           'Start Weight:': start_weight,
                                           'Stop Weight:': stop_weight,
                                           'Realized Weight Gain:': fcc_gain,
                                           'Average Daily Gain:': adg,
                                           'Total Ration:': total_ration,
                                           'Daily Ration:': daily_ration,
...
#    st.download_button(label='Download CSV', data=csv, file_name='sample.csv', mime='text/csv')
